...
'''class DiscriminativeSubNetwork(nn.Module):
    def __init__(self, channel):
        ...
        self.discriminator = nn.Sequential(
            nn.Conv2d(channel, feature_num_2, kernel_size=3, stride=1, padding=1),
            nn.BatchNorm2d(feature_num_2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(feature_num_2, feature_num, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(feature_num),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(feature_num, feature_num_x2, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(feature_num_x2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(feature_num_x2, feature_num_x2, kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(feature_num_x2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Flatten(),
            nn.Linear(4096, 1),  # Assuming input shape after Flatten is 16x16x256 = 4096
            nn.Sigmoid()
        )
...
class Model_D_net(nn.Module):
    def __init__(self):
        super(Model_D_net, self).__init__()
        self.discriminator = nn.Sequential(
            ...
            nn.ReLU(True),
            Flatten(),
            nn.Linear(131072, 1),#bs=4, channel = 1, img.shapae=256*256
            nn.Sigmoid()
        )
